...
async function createInitialProducts(){
    try{
        ...
        const dmfm = await createProduct ({title:'Dial M for Murder',description:'M.',price:45.00,quantity:243,imageUrl:"https://i.ytimg.com/vi/Jl9rsKq1BTQ/movieposter_en.jpg",categoryId:movies.id})
        ...
        // videogames
        const ogMario = await createProduct({title:'Super Mario Bros 3', description:' Mario/Luigi Bros',price:15.00,quantity:120, imageUrl:'https://i.imgur.com/KLDNNeF.png',categoryId:videogames.id})
        const streetFighter = await createProduct({title:'Street Fighter II', description:'Fight fight', price:23.00, quantity:10,imageUrl:'https://i.imgur.com/JEsyUdv.png', categoryId:videogames.id})
        const gta = await createProduct({title:'Grand Theft Auto Five',description:'Steal cars, hit a grandma in the throat', price: 16.00,quantity:12, imageUrl:'https://i.imgur.com/rQUIEVS.png', categoryId:videogames.id})
        ...
        const spaceInv = await createProduct({title:'Space Invaders', description:'INVADE INVADE', quantity:34.00,price:300,imageUrl:'https://i.imgur.com/0ZWYuoA.png',categoryId:videogames.id})
        const tetris = await createProduct({title:'Tetris', description:'Stacks on Stacks',price:21 ,quantity:23.00,imageUrl:'https://i.imgur.com/IkClhGT.png', categoryId:videogames.id})
        const mineCraft= await createProduct({title:'Minecraft', description:'mining a craft', price:23.00, quantity:34, imageUrl:'https://imgur.com/6khS1eJ',categoryId:videogames.id})
        const eldenRing= await createProduct({title:'Elden Ring', description:'Mason', price:21.09, quantity:90,imageUrl:'https://i.imgur.com/pw6DVY6.png',categoryId:videogames.id})
        const skullandbones= await createProduct({title:'Skull and Bones', description:'death',price:90.00, quantity:12,imageUrl:'https://imgur.com/clDgrVD', categoryId:videogames.id})
        const pokemon = await createProduct({title:'Pokemon', description:'Catch them ALL',price:12.00, quantity:90, imageUrl:'https://i.imgur.com/WEGhA6I.png',categoryId:videogames.id})
        const zelda = await createProduct({title:'The Legend of Zelda',description:'A legend', price:43.00,quantity:45, imageUrl:'https://cdn.shopify.com/s/files/1/0630/8509/products/pst1131_Zelda_Breath_of_the-_Wild_BOTW_Poster_large.jpg?v=1608456234',categoryId:videogames.id})
        ...
        const childish = await createProduct({title:'Childish Gambino',description:'Donald Glover',price:80.00,quantity:900,imageUrl:'https://1.bp.blogspot.com/-JXk9I8t89PU/XaVe80f8HQI/AAAAAAABl9E/kXXXhs0hJ2UZZmpD8oy0pRweLX8DLuPJQCLcBGAsYHQ/s1600/Childish%2BGambino%2BAustin%2BPrint%2BZeb%2BLove.jpg',categoryId:music.id})
        const kurtVille= await createProduct({title:'Kurt Vile and the Violators',description:'American singer, songwriter, multi-instrumentalist, and record producer. He is known for his solo work and as the former lead guitarist of rock band the War on Drugs.',price:89.00,quantity:90,imageUrl:'https://mir-s3-cdn-cf.behance.net/project_modules/max_1200/cf863243747699.57fb951c29da8.png',categoryId:music.id})
        const khruangbin= await  createProduct({title:'Khruangbin',description:'Trio from Houston, Texas',price:56.00,quantity:89,imageUrl:'https://cdn.shopify.com/s/files/1/0357/5398/4140/products/SpacesuitsPoster_360x_1024x1024_2x_b8832912-fa80-4458-9515-2b470458627b_9_1024x1024@2x.jpg?v=1653000992',categoryId:music.id})
        const silksonic= await  createProduct({title:'Silk Sonic',description:'Bruno Mars and Anderson Pakk.',price:32.00,quantity:50,imageUrl:'https://i.ebayimg.com/images/g/jXMAAOSwpTFgQ4it/s-l400.jpg', categoryId:music.id})
        const goat = await createProduct({title:'GOAT',description:'Mac Miller in Album Swimming RIP',price:1000000,quantity:90,imageUrl:'https://i.pinimg.com/736x/28/5b/93/285b934a902b3478625fea6dec7c7fdb.jpg',categoryId:music.id})
        const riri = await createProduct({title:'Rihanna',description:'riririirr',price:70.00,quantity:12,imageUrl:'https://cdn.europosters.eu/image/750/posters/rihanna-roses-i11876.jpg',categoryId:music.id})
        const frankOcean = await createProduct({title:'Frank Ocean',description:'Blond',price:30.00,quantity:56,imageUrl:'https://centralsauce.com/wp-content/uploads/2018/08/frankocean_popup_shop.jpg',categoryId:music.id})
        const father = await  createProduct({title:'Father John Misty',description:'Fear Fun',price:67.00,quantity:76,imageUrl:'https://www.prestoimages.net/store20/rd2871/2871_pd2171352_1.jpg',categoryId:music.id})
        const hg = await  createProduct({title:'Hermanos Gutierrez',description:'REALISMO MAGICO TOUR',price:23.00,quantity:90,imageUrl:'https://ih1.redbubble.net/image.4057335924.6153/poster,504x498,f8f8f8-pad,600x600,f8f8f8.jpg',categoryId:music.id})
        const nina = await  createProduct({title:'Nina Simone',description:'The AMAAZING NINA SAMONE',price:78.00,quantity:56,imageUrl:'https://i.ebayimg.com/images/g/KpwAAOSwLEtYk8Wj/s-l500.jpg',categoryId:music.id})
