...
async function createInitialProducts(){
    try{
        ...
        const spaceInv = createProduct({title:'Space Invaders', description:'INVADE INVADE', quantity:34.00,imageUrl:' a559cef86e786068ae156e7686d7c639.jpg'})
        const tetris = createProduct({title:'Tetris', description:'Stacks on Stacks',quantity:23.00,imageUrl:' 138742.jpg', categoryId:videogames.id})
        const mineCraft= createProduct({title:'Minecraft', description:'mining a craft', price:23.00, quantity:34, imageUrl:'81KsM2pcecL._AC_SL1500_.jpg',categoryId:videogames.id})
        const eldenRing= createProduct({title:'Elden Ring', description:'Mason', price:21.09, quantity:90,imageUrl:' 6110RSDn3PL._AC_SL1050_.jpg',categoryId:videogames.id})
        const skullandbones= createProduct({title:'Skull and Bones', description:'death',price:90.00, quantity:12,imageUrl:' skull-and-bones-button-fin-1657218024358.jpg', categoryId:videogames.id})
        const pokemon = createProduct({title:'Pokemon', description:'Catch them ALL',price:12.00, quantity:90, imageUrl:'il_fullxfull.1213514888_olad_grande.jpg',categoryId:videogames.id})
        const zelda = createProduct({title:'The Legend of Zeda',description:'A legend', price:43.00,quantity:45, imageUrl:'https://cdn.shopify.com/s/files/1/0630/8509/products/pst1131_Zelda_Breath_of_the-_Wild_BOTW_Poster_large.jpg?v=1608456234',categoryId:videogames.id})
        const rapture = createProduct({title:'Rapture', description:'huh',price:56.00, quantity:34,imageUrl:'https://ih1.redbubble.net/image.1025786612.1376/poster,504x498,f8f8f8-pad,600x600,f8f8f8.jpg'})
        const imposterS =createProduct({title:'Imposter', description:'Impostersss',price:23.00, quantity:90, imageUrl:'https://imgc.allpostersimages.com/img/posters/among-us-impostor-premium-poster_u-L-F9PHF40.jpg',categoryId:videogames.id})
...
// music
        ...
        const  tyler = createProduct({title:'Earfquake',description:'Dont Leave its My Fault',price:67.00, quantity:23,imageUrl:"https://i.pinimg.com/originals/a1/5a/5d/a15a5d5e0e12202f78fbc811a6498c08.jpg",categoryId:music.id})
        const theInternet= createProduct({title:'Ego Death',description:'Album Poster',price:85.00,quantity:34,imageUrl:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTjzT8XKAJI6AFn4O9--rcouJxHmbNfmnXRL7Btwc8ihQ5bfDCY1PGHQWMXDNxRPrOauhI&usqp=CAU', categoryId:music.id})
        const childish =createProduct({title:'Childish Gambino',description:'Donald Glover',price:80.00,quantity:900,imageUrl:'https://1.bp.blogspot.com/-JXk9I8t89PU/XaVe80f8HQI/AAAAAAABl9E/kXXXhs0hJ2UZZmpD8oy0pRweLX8DLuPJQCLcBGAsYHQ/s1600/Childish%2BGambino%2BAustin%2BPrint%2BZeb%2BLove.jpg',categoryId:music.id})
        ...
        const khruangbin= createProduct({title:'Khruangbin',description:'Trio from Houston, Texas',price:56.00,quantity:89,imageUrl:'https://cdn.shopify.com/s/files/1/0357/5398/4140/products/SpacesuitsPoster_360x_1024x1024_2x_b8832912-fa80-4458-9515-2b470458627b_9_1024x1024@2x.jpg?v=1653000992',categoryId:music.id})
        const silksonic= createProduct({title:'Silk Sonic',description:'Bruno Mars and Anderson Pakk.',price:32.00,quantity:50,imageUrl:'https://i.ebayimg.com/images/g/jXMAAOSwpTFgQ4it/s-l400.jpg', categoryId:music.id})
        const goat =createProduct({title:'GOAT',description:'Mac Miller in Album Swimming RIP',price:1000000,quantity:90,imageUrl:'https://i.pinimg.com/736x/28/5b/93/285b934a902b3478625fea6dec7c7fdb.jpg',categoryId:music.id})
        const riri =createProduct({title:'Rihanna',description:'riririirr',price:70.00,quantity:12,imageUrl:'https://cdn.europosters.eu/image/750/posters/rihanna-roses-i11876.jpg',categoryId:music.id})
        const frankOcean = createProduct({title:'Frank Ocean',title:'Blond',price:30.00,quantity:56,imageUrl:'https://centralsauce.com/wp-content/uploads/2018/08/frankocean_popup_shop.jpg',categoryId:music.id})
        const father = createProduct({title:'Father John Misty',description:'Fear Fun',price:67.00,quantity:76,imageUrl:'https://www.prestoimages.net/store20/rd2871/2871_pd2171352_1.jpg',categoryId:music.id})
        const hg = createProduct({title:'Hermanos Gutierrez',description:'REALISMO MAGICO TOUR',price:23.00,quantity:90,imageUrl:'https://ih1.redbubble.net/image.4057335924.6153/poster,504x498,f8f8f8-pad,600x600,f8f8f8.jpg',categoryId:music.id})
        const nina = createProduct({title:'Nina Simone',description:'The AMAAZING NINA SAMONE',price:78.00,quantity:56,imageUrl:'https://i.ebayimg.com/images/g/KpwAAOSwLEtYk8Wj/s-l500.jpg',categoryId:music.id})
